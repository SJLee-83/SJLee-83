...
> 어르신을 위한 무인 이동 서비스를 소형 RC카(Jetson Orin Nano)로 구현한 SSAFY 공통프로젝트입니다. 실내 5×3m 트랙에서 호출 → 배차 → 주행 → 도착 보고까지 전 과정을 무인으로 수행하며, 최종 시연에서 **사람 개입 없이 완주 3회**(도착 오차 최소 8.7cm)를 기록했습니다.
...
- **특징:**
  - **모든 외부 연결에 대체 동작을 설계했습니다.** 비전이 멈추면 GPS 단독으로 계속 주행하고, GPS가 끊기면 추측항법으로 한도까지 버틴 뒤 정지하며, 관제가 끊기면 3초 내 스스로 정지합니다.
  ...
  - 테스트 383건과, GPS 서버·관제 서버·차량 전체를 실제 통신으로 묶은 통합 시뮬레이션으로 실차 없이 PC에서 전 시나리오를 검증했습니다.
