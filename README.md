...
> 어르신을 위한 무인 이동 서비스를 소형 RC카(Jetson Orin Nano)로 구현한 SSAFY 공통프로젝트로, 실내 5×3m 트랙에서 호출·배차·주행·도착 보고까지 사람 개입 없이 수행합니다. 시연 당일 완주 3회(점검 주행 2회, 본 시연 1회), 본 시연 도착 오차 12.2cm와 8.7cm를 기록했습니다.
...
- **한계:** 시연 완주는 비전을 끈 GPS 단독 모드로 달성했고, 비전 차선 보조는 보정 방향 오류가 미해결로 남아 원인과 개선 방향을 저장소 README 트러블슈팅 절에 정리했습니다.
...
- **특징:**
  ...
  - 의도 분류 LLM의 출력은 허용 목록 두 개(`제품 추천` · `단순 대화`)와 대조해 벗어나면 기본값으로 폴백하며, 호출이 실패해도 같은 기본값으로 진행합니다.
...
- **특징:**
  - 지표 라이브러리(ta-lib) 없이 pandas·numpy로 이동평균(5/20/60/120일), 볼린저밴드(20일 2σ), RSI(Wilder 지수평활), 베타(공분산 기반) 계산식을 작성했습니다.
  - 실루엣 5구간은 52주 고저 대비 현재가 위치를 5단계로 분류하고, 저점 구간인데 RSI가 과매수면 경고를 덧붙입니다.
  - 외부 데이터 수집은 live 실패 시 TTL이 지난 캐시, mock JSON 순으로 폴백하고 그마저 없을 때만 503을 반환합니다. AI 인사이트는 Gemini 호출·파싱이 실패해도 mock으로 폴백하며 AI 생성물 면책을 함께 표시합니다.
