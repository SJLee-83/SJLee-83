...
> 어르신을 위한 무인 이동 서비스를 소형 RC카(Jetson Orin Nano)로 구현한 SSAFY 공통프로젝트입니다. 실내 5×3m 트랙에서 호출, 배차, 주행, 도착 보고까지 사람 개입 없이 수행합니다. 시연 당일 완주 3회(점검 주행 2회, 본 시연 1회), 본 시연 도착 오차 12.2cm와 8.7cm를 기록했습니다.
...
- **특징:**
  - 외부 연결 세 곳에 각각 대체 동작을 두었습니다. 비전이 멈추면 GPS 단독으로 주행을 계속하고, 관제가 끊기면 3초 안에 정지합니다. GPS 좌표가 끊기면 0.6초 뒤 정지하되, 유실이 잦은 지정 구역에서는 추측항법으로 1.2m까지 이어간 뒤 정지합니다.
  - 모든 구동 명령은 SafetySupervisor 한 곳을 거칩니다. 주행 프로세스 전체가 멈추는 경우는 별도 프로세스가 하트비트 파일을 감시해 0.7초 이상 갱신이 없으면 I2C로 모터 전원을 차단합니다. 이 가드는 프로세스 동결로 차량이 트랙을 이탈한 뒤 추가했습니다.
  - 조향이 지령각의 약 40%만 반영되는 편차는 원 주행으로 회전 반경을 실측해 계량하고, 회전 제어를 실측 반경 기반 고정 조향으로 바꿔 완주했습니다.
  - 테스트 383건과 통합 시뮬레이션(GPS 서버, 자체 관제 서버, 합성 카메라, 차량 전 스택을 실제 통신으로 연결)으로 실차 없이 PC에서 전 시나리오를 검증했습니다.
- **한계:** 시연 완주는 비전을 끈 GPS 단독 모드로 달성했고, 비전 차선 보조는 보정 방향 오류가 미해결로 남았습니다. 원인과 개선 방향은 저장소 README 트러블슈팅 절에 있습니다.
...
- **역할 (2인 팀):** RA6E1 주행 펌웨어(SPI Slave, I2C 모터·서보), 초음파 센서 기반 충돌 회피, 관제 GUI에서 MQTT와 ESP32를 거쳐 SPI로 이어지는 제어 체인 전체를 맡았습니다. 별도로 MCUboot 기반 서명 검증 OTA를 독립 시도했습니다.
  - 팀원 담당: AI 음성 제어, Dual-Bank OTA(최종 시연 채택)
- **특징:**
  - ESP32와 RA6E1 사이에 BUSY GPIO 핸드셰이크를 두어, 슬레이브가 처리 중일 때 마스터가 다음 바이트를 보내지 못하게 했습니다. SPI 자체에는 흐름 제어가 없기 때문입니다.
  - 초음파 왕복시간은 DWT 사이클 카운터로 μs 단위 계측했습니다(타이머 채널 절약). 센싱은 비차단 상태머신으로 돌려 SPI 통신과 동시에 처리합니다.
  - 관제 GUI의 MQTT 콜백은 Signal/Slot으로 메인 스레드에 넘겨 처리합니다. 콜백 스레드에서 위젯을 직접 건드려 발생하던 크래시를 이 방식으로 없앴습니다.
- **한계:** MCUboot 작업 범위는 FSP 설정, 슬롯과 서명 파이프라인 구성, 실기 검증입니다. 서명 검증, 슬롯 교체, 부팅까지는 실기로 확인했으나 무선 전송은 미완입니다. 실행 중인 코드 플래시에 쓰면 다음 명령을 fetch하지 못해 hang되는 self-programming 제약 때문이며, 팀이 채택한 Dual-Bank 방식은 이 제약을 하드웨어 뱅크 전환으로 우회합니다.
...
> 유지보수 매뉴얼 기반 RAG와 LangGraph 조건부 라우팅으로, 설비 알람 코드에 대한 한국어 조치 가이드를 안전 주의사항부터 출처와 함께 생성합니다.

- **역할:** 스마트 제조 AI Agent 해커톤 2025(DACON) 본선 진출작(팀)의 기획과 MVP 설계를 리드했고(구현은 팀원 주도), 그 개념을 계승해 기술 스택과 데이터를 검증한 뒤 코드는 처음부터 단독으로 재구현했습니다.
- **특징:**
...
- **특징:**
  ...
  - 의도 분류 LLM의 출력은 허용 목록 두 개(`제품 추천` · `단순 대화`)와 대조해 벗어나면 기본값으로 폴백합니다. 호출이 실패해도 같은 기본값으로 진행합니다.
  - 제품 검색은 pgvector `<=>` 코사인 거리 쿼리를 직접 작성했고, 피부 데이터와 질문을 임베딩해 제품 3종을 추출합니다.
- **한계:** 테스트가 없어, 리팩터링 중 상태 전이 키(`button_text`) 누락으로 2개 상태가 도달 불가해진 회귀 버그를 뒤늦게 발견해 수정했습니다(2026-07-12). 추천 품질에 대한 정량 지표도 없습니다.
